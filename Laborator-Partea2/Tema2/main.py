...
class Joc:
    ...
    def piesaAparata(self, i, j):
        ...
        for per_dir in directii:
            len_culoare = self.parcurgere(directie=per_dir[0], jucator=jucator) + self.parcurgere(directie=per_dir[1], jucator=jucator) + 1  # +1 pt chiar ultima mutare
            if len_culoare >= 3:
    ...
    def valid(self, i, j):
        ...
        if (self.matr[i][j] == Joc.JMAX and self.ultima_mutare_JMAX[0] == i and self.ultima_mutare_JMAX[1]) or (self.matr[i][j] == Joc.JMIN and self.ultima_mutare_JMIN[0] == i and self.ultima_mutare_JMIN[1]):
            return False
        elif self.piesaAparata(i, j):
            return False
        return True
    ...
    def deseneaza_posibilitati(self, status=0, pozitii=None):
        # GEN POZITIILE LIBERE DE LANGA
        # TO DO : de adaugat piesele de jucator opus care sunt capturabile
            ...
            if event.type == pygame.QUIT:
                ...
                for np in range(len(Joc.celuleGrid)):
                    ...
                    if Joc.celuleGrid[np].collidepoint(pos):
                        ...
                        # ++ linii de la mine
                        elif stare_curenta.tabla_joc.matr[linie][coloana] == Joc.JMAX and stare_curenta.tabla_joc.matr.valid(linie,coloana):
                            if de_mutat and (
                            ...
                            if de_mutat:
                                ...
                                stare_curenta.tabla_joc.matr[
                                    de_mutat[0] * M + de_mutat[1]] = Joc.GOL
                                de_mutat = False
